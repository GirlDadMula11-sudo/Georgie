...
async function localMailboxBatch(job){
  ...
  const observed=validateNeoObservation(JSON.parse(await runJxa(script)||"{}"),mailboxes),batchId=`mbxbatch_${sha256(`${objectiveId}:${job.id}:${Date.now()}`).slice(0,32)}`,packets=[];
