...
function neoStartStaticContractInspection() {
  const state={status:"running",startedAt:new Date().toISOString(),bundles:[],sourceMaps:[],contracts:[],stores:[],routeResolutions:[],errors:[],credentialsTransferred:false,mailboxDataAccessed:false,mailboxInteractionPerformed:false};window.__georgieNeoStaticContracts=state;
  const forbidden=/token|secret|password|authorization|session|cookie|credential/i,allowedBundle=/\/static\/js\/(?:mail\.[a-f0-9]+|thread-quick-action-list\.[a-f0-9]+(?:\.chunk)?)\.js$/i,anchors=["api.flockmail.com","bll.flockmail.com","/ae/ws/create"];
  const hash=async value=>{const bytes=new TextEncoder().encode(value),digest=await crypto.subtle.digest("SHA-256",bytes);return{bytes:bytes.length,sha256:[...new Uint8Array(digest)].map(item=>item.toString(16).padStart(2,"0")).join("")}};
  const analyze=(source,bundlePath)=>{for(const anchor of anchors){let offset=0,count=0;while((offset=source.indexOf(anchor,offset))>=0&&count++<8){const context=source.slice(Math.max(0,offset-40000),Math.min(source.length,offset+40000)),routes=[...context.matchAll(/["'](\/[A-Za-z0-9_~.?&=:+{}$()\[\],@!%*-]{2,300})["']/g)].map(match=>match[1]).filter(value=>!forbidden.test(value)&&/mail|message|thread|conversation|inbox|search|sync|fetch|list|ae\//i.test(value));state.routeResolutions.push({anchor,bundlePath,occurrence:count,routeTemplates:[...new Set(routes)].slice(0,80),methods:[...new Set((context.match(/\b(?:GET|HEAD|POST|PUT|PATCH|DELETE)\b/g)||[]))].sort(),immutableIdFields:[...new Set((context.match(/\b(?:messageId|message_id|mailId|mail_id|threadId|thread_id|conversationId|conversation_id|uid|messageUid|threadUid)\b/gi)||[]))].slice(0,40)});offset+=anchor.length}}};
  ...
  Promise.allSettled(resources.map(async raw=>{const url=new URL(raw,location.href),response=await fetch(url.href,{method:"GET",credentials:"omit",cache:"no-store",redirect:"error"}),source=await response.text();if(!response.ok||source.length>8000000)throw new Error("bundle fetch rejected");state.bundles.push({path:url.pathname,...await hash(source)});analyze(source,url.pathname);const mapRef=(source.match(/[#@]\s*sourceMappingURL=([^\s*]+)/)?.[1]||"").trim();if(mapRef&&!mapRef.startsWith("data:")){const mapUrl=new URL(mapRef,url);if(mapUrl.origin!==location.origin||mapUrl.protocol!=="https:"||!mapUrl.pathname.endsWith(".map"))throw new Error("source map origin rejected");const mapResponse=await fetch(mapUrl.href,{method:"GET",credentials:"omit",cache:"no-store",redirect:"error"}),mapText=await mapResponse.text();if(!mapResponse.ok||mapText.length>12000000)throw new Error("source map fetch rejected");const parsed=JSON.parse(mapText),joined=(parsed.sourcesContent||[]).filter(value=>typeof value==="string").join("\n").slice(0,12000000);state.sourceMaps.push({path:mapUrl.pathname,...await hash(mapText),sources:Array.isArray(parsed.sources)?parsed.sources.length:0,hasSourcesContent:Boolean(joined)});analyze(joined,mapUrl.pathname)}for(const [kind,pattern] of [["indexeddb",/\bindexedDB\b/],["service_worker",/\bserviceWorker\b/],["websocket",/\bWebSocket\b/],["cache_storage",/\bcaches\b/]])if(pattern.test(source))state.stores.push({kind,bundlePath:url.pathname})})).then(results=>{state.errors.push(...results.filter(item=>item.status==="rejected").map(item=>String(item.reason?.message||item.reason).slice(0,240)));state.routeResolutions=state.routeResolutions.slice(0,40);state.status="completed";state.completedAt=new Date().toISOString()});
...
function neoStartReadOnlyApiProbe(){
  const state={status:"running",startedAt:new Date().toISOString(),records:[],sources:[],responseSchemas:[],webSockets:[],mutations:[],errors:[],credentialsExported:false,requestBodiesCaptured:false,messageBodiesRecorded:false,personalContentRecorded:false};window.__georgieNeoApiProbe=state;
  ...
  const schema=(value,source)=>{if(!value||typeof value!=="object")return;const sample=Array.isArray(value)?value.find(item=>item&&typeof item==="object"):value;if(!sample)return;const fields=Object.keys(sample).filter(key=>!forbidden.test(key)).slice(0,120),idFields=fields.filter(key=>/(?:^|_)(?:message|mail|thread|conversation)?_?(?:id|uid)$/i.test(key));state.responseSchemas.push({...source,fields,idFields})};
  const inspect=async(response,method,raw)=>{const source=safeUrl(raw);if(!source)return;if(!state.sources.some(item=>JSON.stringify(item)===JSON.stringify({...source,method})))state.sources.push({...source,method});if(!["GET","HEAD"].includes(method)){if(mailish.test(source.path))state.mutations.push({...source,method});return}const type=String(response.headers?.get?.("content-type")||""),length=Number(response.headers?.get?.("content-length")||0);if(type.includes("json")&&(!length||length<=3000000)){try{const value=await response.clone().json();schema(value,source);walk(value,source)}catch(error){state.errors.push(String(error?.message||error).slice(0,200))}}};
  ...
  if(window.WebSocket){const NativeWebSocket=window.WebSocket;window.WebSocket=function(raw,protocols){let safe=null;try{const url=new URL(String(raw||""));if(url.protocol==="wss:"&&!forbidden.test(url.pathname))safe={origin:url.origin,path:url.pathname}}catch{}if(safe)state.webSockets.push({...safe,created:true,payloadCaptured:false});return protocols===undefined?new NativeWebSocket(raw):new NativeWebSocket(raw,protocols)};window.WebSocket.prototype=NativeWebSocket.prototype;Object.assign(window.WebSocket,{CONNECTING:0,OPEN:1,CLOSING:2,CLOSED:3})}
...
function neoMailboxObserver(mailbox, cursors, max, apiProbe) {
  ...
  for (const row of rows) {
    ...
    const {messageId,threadId,messageIdSource,threadIdSource,captured}=ids;
    ...
    row.setAttribute("data-georgie-message-id", messageId);
    messages.push({ mailbox, messageId, threadId, messageIdSource, threadIdSource, timestamp, sender: addresses[0] || "", recipients: [], subject, rowExcerpt: rowText, readStateBefore: readState, sourceUrl: text(location.origin + location.pathname, 1000), capturedContent: captured?.content || "", capturedBodyComplete: captured?.bodyComplete === true, capturedSourceOrigin: captured?.sourceOrigin || "", capturedSourceEndpoint: captured?.sourceEndpoint || "", capturedSourceMethod: captured?.sourceMethod || "", capturedIdentifierFields: captured?.identifierFields || null });
...
function neoGuardedMessageOpener(messageId) {
  ...
  const candidates = [...document.querySelectorAll("[data-georgie-message-id],[data-message-id],[data-id],[data-thread-id],[role='row']")].filter(element => {
    const values = [element.getAttribute("data-georgie-message-id"), element.getAttribute("data-message-id"), element.getAttribute("data-id"), element.getAttribute("data-thread-id"), element.id].map(text);
