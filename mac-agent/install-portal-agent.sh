...
PLIST="$HOME/Library/LaunchAgents/com.georgie.portal-agent.plist";LOG="$HOME/Library/Logs";HEALTH="$HOME/Library/Application Support/Georgie/portal-agent-health.json";mkdir -p "${PLIST:h}" "$LOG" "${HEALTH:h}";rm -f "$HEALTH"
cat > "$PLIST" <<PLIST
...
for _ in {1..35};do if [[ -f "$HEALTH" ]]&&"$NODE" -e 'const f=require("fs"),h=JSON.parse(f.readFileSync(process.argv[1]));if(h.deviceId!=="primary-mac-portal"||h.workerVersion!=="1.1.0"||h.lastPollOk!==true||Date.now()-Date.parse(h.successfulCycleAt)>35000)process.exit(1)' "$HEALTH";then echo "Governed portal worker v1.1.0 installed and fresh adaptive polling proven.";exit 0;fi;sleep 1;done
echo "Portal worker fresh heartbeat proof failed";launchctl print "$TARGET" 2>&1|tail -40||true;tail -40 "$LOG/georgie-portal-agent-error.log" 2>/dev/null||true;exit 1
