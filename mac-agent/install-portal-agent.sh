...
for _ in {1..25};do if [[ -f "$HEALTH" ]]&&"$NODE" -e 'const f=require("fs"),h=JSON.parse(f.readFileSync(process.argv[1]));if(h.deviceId!=="primary-mac-portal"||h.workerVersion!=="1.0.0"||h.lastPollOk!==true||Date.now()-Date.parse(h.successfulCycleAt)>15000)process.exit(1)' "$HEALTH";then echo "Governed portal worker installed and fresh polling proven.";exit 0;fi;sleep 1;done
