...
test("full runtime starts recurring SEO scheduler",()=>{assert.ok(componentsForProfile("web", undefined, null, "full").some(component=>component.id==="seo-monitor"));});
...
test("production verifies materialized SEO layers and maintenance retains installers",()=>{const pkg=JSON.parse(fs.readFileSync(new URL("../package.json",import.meta.url),"utf8"));assert.match(pkg.scripts.prestart,/verify-runtime-baseline\.mjs/);for(const s of [pkg.scripts.check]){assert.match(s,/install-autonomous-seo-ops\.mjs/);assert.match(s,/install-autonomous-seo-ops-round2\.mjs/);}});
