...
test("terminal reconciliation installer accepts the bounded automatic recovery contract", () => {
  ...
  const supervisor = fs.readFileSync(new URL("../scripts/install-connector-recovery-supervisor.mjs", import.meta.url), "utf8");
  ...
  assert.match(supervisor, /modernBoundedRecovery/);
  assert.match(supervisor, /CONNECTOR_RECOVERY_SUPERVISOR_BOUNDED_RECOVERY_MISSING/);
