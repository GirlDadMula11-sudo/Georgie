...
test("screenshot-only Roblox play-test failure recovers the same identity",async()=>{
  ...
  const recovered=await recoverLongRunningMacJob(deviceId,job.id,{expectedAction:"roblox.play_test_validate",requiredAgentVersion:"2.2.52"});
  assert.equal(recovered.id,job.id);
  assert.equal(recovered.status,"queued");
  assert.equal(recovered.resumeCount,1);
  assert.equal(recovered.args.requiredAgentVersion,"2.2.52");
...
test("blank Place1 Roblox play-test block recovers the same identity for native File Open repair",async()=>{
  ...
  assert.equal(recovered.resumeCount,1);
  assert.equal(recovered.args.requiredAgentVersion,"2.2.52");
  assert.equal(recovered.resumeHistory.at(-1).reason,"play_test_compiled_scpt_execution_repaired");
});
