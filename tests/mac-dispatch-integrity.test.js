...
test("approved missing Makayla play-test ledger entry is restored once under the exact identity",async()=>{
  ...
  await assert.rejects(()=>recoverLongRunningMacJob("primary-mac",jobId,{expectedAction:"roblox.play_test_validate",requiredAgentVersion:"2.2.65",governance:{...governance,approvalId:"wrong"}}),/MAC_LONG_RUNNING_RESTORATION_APPROVAL_REJECTED/);
  await assert.rejects(()=>recoverLongRunningMacJob("primary-mac",jobId,{expectedAction:"roblox.play_test_validate",requiredAgentVersion:"2.2.65",governance:{...governance,idempotencyKey:"approval:wrong"}}),/MAC_LONG_RUNNING_RESTORATION_APPROVAL_REJECTED/);
  const first=await recoverLongRunningMacJob("primary-mac",jobId,{expectedAction:"roblox.play_test_validate",requiredAgentVersion:"2.2.65",governance});
  const second=await recoverLongRunningMacJob("primary-mac",jobId,{expectedAction:"roblox.play_test_validate",requiredAgentVersion:"2.2.65",governance});
...
test("missing Makayla identity restoration rejects scope expansion",async()=>{
  ...
  assert.equal(await recoverLongRunningMacJob("primary-mac","idem-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",{expectedAction:"roblox.play_test_validate",requiredAgentVersion:"2.2.65",governance:{planId,approvalId,idempotencyKey:`approval:${approvalId}:plan:${planId}`}}),null);
});
