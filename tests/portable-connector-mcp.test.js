...
test("portable connector exposes three focused accurately annotated tools", () => {
  assert.deepEqual(GEORGIE_CONNECTOR_TOOLS.map((tool) => tool.name), ["georgie_dispatch_command", "georgie_forward_approval", "georgie_get_command"]);
...
test("MCP initializes, lists tools, dispatches, forwards approvals, and reads status", async () => {
  ...
  const listed = await handle({ jsonrpc: "2.0", id: 2, method: "tools/list" }); assert.equal(listed.result.tools.length, 3);
