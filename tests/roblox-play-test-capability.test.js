...
test("exact play-test recovery marker preserves the completed job identity", () => {
  const request = {jobId:playTestJobId,deviceId:"primary-mac",expectedAction:"roblox.play_test_validate",projectRoot,requiredAgentVersion:"2.2.49"};
...
test("play-test recovery marker rejects identity, path, action, or version expansion", () => {
  const valid = {jobId:playTestJobId,deviceId:"primary-mac",expectedAction:"roblox.play_test_validate",projectRoot,requiredAgentVersion:"2.2.49"};
...
test("exact play-test marker prepares one non-publishing plan", () => {
  const marker = `ROBLOX_PLAY_TEST_JSON: ${JSON.stringify({projectRoot,requiredAgentVersion:"2.2.49"})}`;
...
test("play-test marker rejects path or agent expansion", () => {
  for (const request of [
    {projectRoot:"/Users/mac/Documents/Other",requiredAgentVersion:"2.2.49"},
