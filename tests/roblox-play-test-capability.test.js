...
test("Mac agent performs a bounded Studio play test with six gameplay checks", () => {
  ...
  assert.match(agent, /openRobloxStudioArtifactDirectly/);
  assert.match(agent, /ROBLOX_STUDIO_UNRELATED_DOCUMENT_OPEN/);
  assert.match(agent, /rawStudioDocument is not missing value/);
  assert.match(agent, /conflictingPathlessWindows/);
  assert.match(agent, /studioWindowTitle does not contain "Open Roblox File"/);
  assert.match(agent, /key code 53/);
  assert.match(agent, /execFileAsync\("\/usr\/bin\/pkill", \["-TERM", "-x", "RobloxStudio"\]/);
  assert.match(agent, /tell application "RobloxStudio" to quit'\)\.catch\(\(\) => \{\}\)/);
  assert.match(agent, /pathToFileURL\(expectedArtifact\)\.href/);
  assert.match(agent, /execFileAsync\("\/usr\/bin\/open", \["-n", "-a", "RobloxStudio", expectedArtifact\]/);
  assert.match(agent, /execFileAsync\("\/usr\/bin\/open", \["-a", "RobloxStudio", expectedArtifact\]/);
  assert.match(agent, /studioDocument is expectedArtifactUrl/);
  assert.match(agent, /studioDocument is expectedArtifactDecodedUrl/);
  assert.match(agent, /studioDocument is expectedArtifactLocalhostUrl/);
  assert.match(agent, /studioDocument is expectedArtifactDecodedLocalhostUrl/);
  assert.match(agent, /studioDocument is not expectedArtifactUrl/);
  assert.doesNotMatch(agent, /studioWindowTitle contains "Prototype\.rbxlx"/);
  assert.match(agent, /direct_file_url_attachment_wait/);
  assert.match(agent, /launch_services_file_url_attachment/);
  assert.match(agent, /studioFileOpenAttachmentAttempts/);
  assert.match(agent, /direct_document_verified/);
  assert.match(agent, /direct_launch_services/);
  ...
  assert.match(agent, /set openButton to my findOpenButtonRecursively\(openPanel, 5\)/);
  ...
  assert.match(agent, /open_keyboard_default_press/);
  assert.match(agent, /open_keyboard_close_wait/);
  assert.match(agent, /keyboard_default_return/);
  ...
  assert.match(agent, /collectAccessibilityTopology\(diagnosticPanel, 6, 40\)/);
  assert.ok(fileDialog.indexOf("open_keyboard_default_press") < fileDialog.indexOf("open_button_nested_scan"), "native Return must precede the bounded accessibility fallback");
  assert.match(fileDialog, /go_to_folder_return_confirm/);
  assert.match(fileDialog, /go_to_folder_second_return_confirm/);
  assert.match(fileDialog, /if goSheetStillOpen then error "ROBLOX_STUDIO_GO_TO_FOLDER_SHEET_STUCK"/);
  assert.ok(fileDialog.indexOf("go_to_folder_second_return_confirm") < fileDialog.indexOf("open_keyboard_default_press"), "the Go to Folder sheet must be proven closed before the outer Open action");
  ...
  assert.match(tools, /requiredAgentVersion:"2\.2\.65"/);
  ...
  assert.ok(activation.indexOf("openRobloxStudioArtifactDirectly(artifact)") < activation.indexOf("openRobloxStudioArtifactThroughFileDialog(artifact)"), "Studio's internal File > Open must follow the bounded direct-open attempt");
  assert.ok(activation.indexOf("openRobloxStudioArtifactThroughFileDialog(artifact)") < activation.indexOf("if (!studioWindow.ready) return"), "the internal File > Open fallback must run before the exact-document gate blocks playback");
  assert.match(activation, /directOpenStage === "direct_file_url_attachment_wait"/);
  assert.match(activation, /studioWindow = await waitForRobloxStudioArtifactWindow\(artifact, 15000\)/);
...
test("exact play-test recovery marker preserves the completed job identity", () => {
  const request = {jobId:playTestJobId,deviceId:"primary-mac",expectedAction:"roblox.play_test_validate",projectRoot,requiredAgentVersion:"2.2.65"};
...
test("play-test recovery marker rejects identity, path, action, or version expansion", () => {
  const valid = {jobId:playTestJobId,deviceId:"primary-mac",expectedAction:"roblox.play_test_validate",projectRoot,requiredAgentVersion:"2.2.65"};
...
test("exact play-test marker prepares one non-publishing plan", () => {
  const marker = `ROBLOX_PLAY_TEST_JSON: ${JSON.stringify({projectRoot,requiredAgentVersion:"2.2.65"})}`;
...
test("play-test marker rejects path or agent expansion", () => {
  for (const request of [
    {projectRoot:"/Users/mac/Documents/Other",requiredAgentVersion:"2.2.65"},
