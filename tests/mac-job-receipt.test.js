...
test("exact long-running recovery marker prepares one identity-preserving plan",()=>{
  const marker=`MAC_LONG_RUNNING_RECOVERY_JSON: ${JSON.stringify({jobId:recoveryJobId,deviceId:"primary-mac",expectedAction:"roblox.install_rojo_and_build",requiredAgentVersion:"2.2.61"})}`;
  const [action]=deterministicToolPlan(marker);
  assert.equal(action.tool,"approvals.prepare_plan");
  assert.deepEqual(action.args.execution.args,{jobId:recoveryJobId,deviceId:"primary-mac",expectedAction:"roblox.install_rojo_and_build",requiredAgentVersion:"2.2.61"});
...
test("long-running recovery marker rejects any scope expansion",()=>{
  for(const request of [
    {jobId:recoveryJobId,deviceId:"secondary-mac",expectedAction:"roblox.install_rojo_and_build",requiredAgentVersion:"2.2.61"},
    {jobId:recoveryJobId,deviceId:"primary-mac",expectedAction:"roblox.install_rojo_and_build",requiredAgentVersion:"2.2.41"},
    {jobId:recoveryJobId,deviceId:"primary-mac",expectedAction:"roblox.create_job",requiredAgentVersion:"2.2.61"}
