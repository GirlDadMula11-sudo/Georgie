...
test("NEO observation script is exact-identity, bounded, and no-click",()=>{
  ...
  assert.doesNotMatch(script,/\.click\s*\(/);assert.doesNotMatch(script,/location\s*=/);assert.doesNotMatch(script,/fetch\s*\(/);assert.doesNotMatch(script,/XMLHttpRequest/);
...
test("NEO observation validation fails closed on ambiguity, missing identity, or mutation",()=>{
  ...
  assert.equal(validateNeoObservation({provider:"neo_browser",navigationPerformed:false,mailboxMutation:false,mailboxes:connected,messages:[]},mailboxes).provider,"neo_browser");
  assert.throws(()=>validateNeoObservation({provider:"neo_browser",navigationPerformed:false,mailboxMutation:false,mailboxes:{},messages:[]},mailboxes),/IDENTITY_NOT_VERIFIED/);
  assert.throws(()=>validateNeoObservation({provider:"neo_browser",navigationPerformed:true,mailboxMutation:false,mailboxes:connected,messages:[]},mailboxes),/READ_ONLY_PROOF_FAILED/);
});
