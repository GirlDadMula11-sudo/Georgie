...
test("NEO observation script supports exact multi-account identity and guarded full-body retrieval",()=>{
  ...
  assert.match(script,/exact_envelope_bound_account_rail/);assert.doesNotMatch(script,/innerWidth \*/);
  assert.match(script,/root\.body\?\.innerText/);assert.match(script,/a\.left - b\.left/);
