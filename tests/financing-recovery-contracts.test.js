...
test("synthetic portal review is preview-only and never touches the store", async () => {
  ...
  try {
    ...
    assert.equal(response.status, 200); assert.equal(body.session.reviewMode, true);
    assert.equal(body.session.businessName, "Sierra Review Company"); assert.equal(body.session.slots.length, 2); assert.equal(calls, 0);
  } finally {
    await new Promise(resolve => server.close(resolve));
