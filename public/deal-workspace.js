...
function renderWorkspace(workspace) {
  ...
  view.innerHTML = `
    ...
    <div class="deal-directive"><span>NEXT BEST ACTION</span><strong>${esc(workspace.nextAction)}</strong></div>
    ...
    <div class="deal-grid">
      ...
      <article><header><span>UNDERWRITING</span><b>${esc(workspace.underwriting?.status || "unknown")}</b></header><p>Confidence: ${workspace.underwriting?.confidence == null ? "Unknown" : percent(workspace.underwriting.confidence)}</p><small>${workspace.underwriting?.evidence?.length || 0} source record(s)</small></article>
