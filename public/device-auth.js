...
async function boot(){if(!token){showEnrollment();return;}try{const response=await fetch("/api/mobile/device",{headers:authHeaders()});if(response.ok){retryAttempt=0;resolveReady(true);return;}let payload={};try{payload=await response.json();}catch{}if(isDefinitiveDeviceRejection(response.status,payload?.code)){localStorage.removeItem("georgie:deviceToken");token="";showEnrollment();return;}}catch{}retryAuthentication();}
boot();
