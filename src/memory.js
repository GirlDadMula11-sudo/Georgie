...
export async function appendSessionTurn({userId="primary",sessionId="default",role,content}){const id=normalizeUserId(userId);const sid=String(sessionId||"default").slice(0,150);const store=await readStore(id);const key=`${id}:${sid}`;const session=store.sessions[key]||{userId:id,sessionId:sid,turns:[],createdAt:now()};session.turns.push({role,content:String(content||"").slice(0,12000),at:now()});session.turns=session.turns.slice(-1000);session.updatedAt=now();store.sessions[key]=session;await writeStore(id,store);return session;}
export async function getSessionHistory(userId="primary",sessionId="default",limit=16){const id=normalizeUserId(userId);const sid=String(sessionId||"default").slice(0,150);const store=await readStore(id);const session=store.sessions[`${id}:${sid}`];if(!session)return[];return session.turns.slice(-Math.max(1,Math.min(1000,Number(limit)||16))).map(({role,content,at})=>({role,content,at}));}
export async function getConversationHistory(userId="primary",limit=200){const id=normalizeUserId(userId);const store=await readStore(id);return Object.values(store.sessions||{}).filter(session=>session?.userId===id).flatMap(session=>(session.turns||[]).map(turn=>({...turn,sessionId:session.sessionId}))).filter(turn=>["user","assistant"].includes(turn.role)&&turn.content).sort((a,b)=>new Date(a.at||0)-new Date(b.at||0)).slice(-Math.max(1,Math.min(2000,Number(limit)||200))).map(({role,content,at,sessionId})=>({role,content,at,sessionId}));}
