...
export async function routeIncomingCommand(userId="primary",input={}){
  ...
  const boundedToolPlan=Array.isArray(input.toolPlan)?input.toolPlan:[];
  const executionObjective=await ensureCanonicalExecutionObjective(userId,{coordinationObjectiveId:objective.objectiveId,title:input.title||input.text,instruction:input.text,metadata:{domain:input.domain,kind:input.kind},toolPlan:boundedToolPlan});
  ...
  const issued=await issueCommand(userId,{objectiveId:objective.objectiveId,issuer:"georgie",assignee:specialist.id,action:authorityAction(input),scope:{domain:input.domain,kind:input.kind,exclusiveResource:`objective:${executionObjective.objectiveId}`,executionObjectiveId:executionObjective.objectiveId},arguments:{instruction:input.text,specialistRole:specialist.role,executionObjectiveId:executionObjective.objectiveId,toolPlan:boundedToolPlan},acceptanceCriteria:["Publish evidence for consequential claims","Return an explicit terminal status","Do not expand scope without a new governed plan"],idempotencyKey});
