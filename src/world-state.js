...
export async function buildWorldState(userId, input = "", route = {}) {
  ...
  const [tasks, events, approvals, decisions] = await Promise.all([listTasks(uid, { status: "open", limit: 100 }), listEvents(uid, { status: "pending", limit: 100 }), listApprovals(uid, { status: "pending", limit: 50 }), listDecisions(uid, { limit: 30, domain: "all" })]);
  ...
  return { version: "2026-08-20.1", generatedAt: new Date().toISOString(), userId: uid, activePacks: selectDomainPacks(input, route.domain), focusDomain: route.domain || "general", commitments, recentDecisions: decisions.slice(0, 10).map(({ id, domain, decision, recommendation, rationale, createdAt }) => ({ id, domain, decision, recommendation, rationale, createdAt })), counts: { openTasks: tasks.length, pendingEvents: events.length, pendingApprovals: approvals.length }, truth: { currentEvidenceRequiredForChangingFacts: true, inferenceMustBeLabeled: true, completedActionsRequireVerification: true }, boundaries: { crossDomainDataRequiresPurpose: true, consequentialActionsRequireApproval: true } };
}
