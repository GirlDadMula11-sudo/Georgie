...
export async function prepareUnifiedOperatingTurn({ userId = "primary", sessionId = "native", input = "" } = {}) {
  ...
  const [continuity, worldState, routing] = await Promise.all([
    ...
    routeIncomingCommand(userId,{...objective,title:objective.text,priority:objective.execution?80:60}).catch((error)=>({unavailable:true,error:error instanceof Error?error.message:String(error),objective:{objectiveId:null},command:null,handoff:null,specialist:null,executionPlan:null}))
  ]);
  ...
  return { version: "unified-georgie-runtime.v4-immutable-plan", preparedAt: new Date().toISOString(), userId: String(userId), sessionId: String(sessionId), objective: {...objective,controlObjectiveId:controlPlane?.objectiveId||null,executionObjectiveId:routing?.executionObjective?.objectiveId||null}, eligibleContinuation, continuity, worldState, controlPlane, specialistRouting:{specialist:routing?.specialist||null,command:routing?.command||null,handoff:routing?.handoff||null,executionPlan:routing?.executionPlan||null}, capabilityManifest, toolSurface: runtimeToolReadiness(toolSurface), executionContract: { loop: ["understand", "freeze_plan", "route_specialist", "claim", "verify_plan_hash", "act", "verify", "publish_evidence", "handoff_or_report", "recover", "learn"], streamProgressImmediately: true, preserveServerWorkAfterDisconnect: true, requireBusinessOutcomeEvidence: true, commonObjectiveIds: true, immutableEvidenceReceipts: true, immutableExecutionPlan:true, ownershipLeases: true, conflictLocks: true, typedCommandEnvelopes: true, specialistWorkers:true, midFlightReplan:false, resultCallbacks: "durable_pull_unless_endpoint_bound", terminalStates: ["completed", "blocked", "approval_needed", "in_progress"], consequentialActionsApprovalGated: true } };
}
