...
import { appendEvidence, prepareObjectiveControlContext, recordCallback } from "./coordination-control-plane.js";
...
export async function prepareUnifiedOperatingTurn({ userId = "primary", sessionId = "native", input = "" } = {}) {
  ...
  const [continuity, worldState, controlPlane] = await Promise.all([
    ...
    prepareObjectiveControlContext(userId,{...objective,title:objective.text,priority:objective.execution?80:60}).catch((error)=>({unavailable:true,error:error instanceof Error?error.message:String(error),objectiveId:null}))
  ]);
  ...
  return { version: "unified-georgie-runtime.v2-control-plane", preparedAt: new Date().toISOString(), userId: String(userId), sessionId: String(sessionId), objective: {...objective,controlObjectiveId:controlPlane?.objectiveId||null}, eligibleContinuation, continuity, worldState, controlPlane, capabilityManifest, toolSurface: runtimeToolReadiness(toolSurface), executionContract: { loop: ["understand", "plan", "claim", "act", "verify", "publish_evidence", "handoff_or_report", "recover", "learn"], streamProgressImmediately: true, preserveServerWorkAfterDisconnect: true, requireBusinessOutcomeEvidence: true, commonObjectiveIds: true, immutableEvidenceReceipts: true, ownershipLeases: true, conflictLocks: true, typedCommandEnvelopes: true, resultCallbacks: "durable_pull_unless_endpoint_bound", terminalStates: ["completed", "blocked", "approval_needed", "in_progress"], consequentialActionsApprovalGated: true } };
}
