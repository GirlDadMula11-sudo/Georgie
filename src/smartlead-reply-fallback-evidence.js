...
export function evaluateSmartleadWebhookThreadFallback({ job, leadId, replyEvent, relatedEvents = [], senderEvidence, reservationExists = false }) {
  ...
  if (relatedEvents.some(e => sameIdentity(e) && lower(e.event_type) === "email_reply" && text(e.provider_message_id) !== text(job.provider_message_id) && time(e.occurred_at) > replyAt + 1000)) return deny("WEBHOOK_FALLBACK_NEWER_INBOUND_EXISTS");
  if (relatedEvents.some(e => sameIdentity(e) && lower(e.event_type) === "email_sent" && time(e.occurred_at) > replyAt + 1000)) return deny("WEBHOOK_FALLBACK_LATER_OUTBOUND_EXISTS");
  const statsId = text(job.provider_stats_id || job.metadata?.provider_stats_id);
  if (!statsId) return deny("WEBHOOK_FALLBACK_STATS_ID_MISSING");
  return { ok: true, reason: "WEBHOOK_FALLBACK_VERIFIED", evidence: { providerCampaignId: text(job.provider_campaign_id), providerLeadId, providerLeadEmail, providerStatsId: statsId, replySender, replyMessageId: text(job.provider_message_id), originalOutboundMessageId: originalOutbound, replyEventId: text(replyEvent.id), providerEventKey: eventKey || expectedEventKey || null, senderMailbox: lower(job.sender_mailbox), replyOccurredAt: new Date(replyAt).toISOString() } };
}
