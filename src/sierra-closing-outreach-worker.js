...
export const SIERRA_CLOSING_OUTREACH_CONTRACT = "georgie.sierra-closing-outreach.v1";
const USER_ID = process.env.GEORGIE_PRIMARY_USER_ID || "primary";
const POLL_MS = bounded(process.env.GEORGIE_SIERRA_CLOSER_POLL_MS, 10_000, 5_000, 300_000);
...
function rows(value) {
  ...
  for (const key of ["deals", "portfolio", "items", "results", "offers", "events", "data"]) if (Array.isArray(value?.[key])) return value[key];
  return [];
}
...
function evidenceRefsOf(offer) {
  const raw = offer?.evidenceRefs || offer?.evidence_refs || offer?.source_event_ids || offer?.evidence_ids || [];
  if (Array.isArray(raw)) return raw.map(value => clean(typeof value === "object" ? value.id || value.ref : value, 300)).filter(Boolean);
  return clean(raw, 300) ? [clean(raw, 300)] : [];
}
function offerIdOf(offer) { return first(offer, ["offerId", "offer_id", "id", "lender_offer_id"]); }
function isVerifiedOffer(offer) {
  const state = first(offer, ["verification_status", "evidence_status", "status"]).toLowerCase();
  return (offer?.verified === true || state === "verified" || state === "authoritative") && Boolean(offerIdOf(offer)) && evidenceRefsOf(offer).length > 0;
}
function offerSnapshot(offers) {
  const identities = offers.map(offer => `${offerIdOf(offer)}:${evidenceRefsOf(offer).sort().join(",")}`).sort();
  return crypto.createHash("sha256").update(identities.join("|")).digest("hex").slice(0, 24);
}
function eventMetadata(event) { return event?.metadata && typeof event.metadata === "object" ? event.metadata : {}; }
function alreadyRecorded(events, idempotencyKey) {
  return rows(events).some(event => {
    const metadata = eventMetadata(event);
    return clean(event?.idempotency_key || metadata.idempotency_key, 300) === idempotencyKey
      || (clean(event?.event_type || event?.type, 120) === "georgie_verified_offer_closing_outreach" && clean(metadata.offer_snapshot, 80) === idempotencyKey.split(":").at(-1));
  });
}
function messageFor(deal) {
  const greeting = firstNameOf(deal) ? `Hi ${firstNameOf(deal)},` : "Hello,";
...
export function evaluateClosingOutreachCandidate({ deal = {}, offers = [], auditEvents = [] } = {}) {
  ...
  const verifiedOffers = rows(offers).filter(isVerifiedOffer);
  if (!verifiedOffers.length) return { eligible:false, reason:"VERIFIED_OFFER_EVIDENCE_MISSING", reference };
  const snapshot = offerSnapshot(verifiedOffers);
  const idempotencyKey = `sierra-closing-outreach:v1:${reference}:${snapshot}`;
...
export async function executeClosingOutreachCandidate({ userId = USER_ID, deal, offers, auditEvents, send = sendMessage, record = recordOutboundCorrespondence, selectMailbox = selectGeorgieCorrespondenceMailbox } = {}) {
  const candidate = evaluateClosingOutreachCandidate({ deal, offers, auditEvents });
  if (!candidate.eligible) return candidate;
  const mailbox = selectMailbox();
  if (!mailbox?.id) throw new Error("SIERRA_CLOSER_MAILBOX_NOT_CONFIGURED");
  const subject = `${businessNameOf(deal)} — financing update ready for review`;
  const text = messageFor(deal);
  const receipt = await send(mailbox.id, {
    to: candidate.clientEmail,
    subject,
    text,
    idempotencyKey: candidate.idempotencyKey,
    correlationId: candidate.idempotencyKey,
    dealId: candidate.reference,
    audience: "client",
    rationale: "Immediately open a non-binding closing conversation after a verified offer reaches the Sierra file.",
    evidenceState: { claims: [{ type:"lender_position", status:"verified" }], evidenceIds: candidate.verifiedOffers.flatMap(evidenceRefsOf) },
  ...
  const crm = await record(userId, { reference:candidate.reference, receipt, message:{ to:candidate.clientEmail, subject, text }, eventType:"georgie_verified_offer_closing_outreach" });
  if (crm?.verification?.ok !== true) throw new Error("SIERRA_CLOSER_CRM_READBACK_INCOMPLETE");
...
export async function runSierraClosingOutreachCycle({ userId = USER_ID, portfolio = getSierraPortfolio, deal = getSierraDeal, offers = getSierraOffers, audit = getSierraAuditEvents, execute = executeClosingOutreachCandidate } = {}) {
  const startedAt = new Date().toISOString(), portfolioResult = await portfolio(userId, { limit:100 }), candidates = rows(portfolioResult).slice(0, 100), results = [];
  for (const summary of candidates) {
    ...
    try {
      const [fullDeal, dealOffers, auditEvents] = await Promise.all([deal(userId, reference), offers(userId, reference), audit(userId, { reference, limit:200 })]);
  ...
  return { contract:SIERRA_CLOSING_OUTREACH_CONTRACT, startedAt, completedAt:new Date().toISOString(), inspected:candidates.length, sent:results.filter(item=>item.status==="sent_verified").length, results };
}
