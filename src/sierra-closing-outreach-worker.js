...
function summaryCanHaveOffer(summary) { const status=statusOf(summary); if([...TERMINAL].some(value=>status.includes(value)))return false; return Number(summary?.available_offers||0)>0||Number(summary?.response_count||0)>0||/offers_received|closing_conditions/i.test(String(summary?.referral_status||"")); }
