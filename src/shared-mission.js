...
export const SHARED_MISSION=Object.freeze({
  ...
  authority:{
    automatic:["observe","diagnose","research","reproduce","run_tests","capture_evidence","create_handoff","retry_temporary_failure","execute_certified_reversible_runbook","commit_verified_patch_to_isolated_branch"],
...
function defaultState(){return{version:1,active:true,mission:SHARED_MISSION,items:[],receipts:[],lastCycleAt:null,createdAt:now(),updatedAt:now()};}
...
async function settle(userId,id,update){const uid=String(userId),state=await missionStatus(uid),item=state.items.find(row=>row.id===id);if(!item)return null;Object.assign(item,update,{lease:null,updatedAt:now()});state.receipts=[...state.receipts,{id:crypto.randomUUID(),handoffId:id,status:item.status,at:now(),summary:bounded(update.summary||update.lastError||"",1000)}].slice(-3000);state.updatedAt=now();await save(uid,state);return item;}
...
export async function deferHandoff(userId,id,{status="waiting_for_capability",result={},summary=""}={}){return settle(userId,id,{status,result,summary});}
