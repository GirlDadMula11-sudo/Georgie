...
router.get("/session",async(req,res)=>{try{const limit=Number(req.query?.limit||80),all=req.query?.scope!=="session";res.json({ok:true,scope:all?"continuous":"session",history:all?await getConversationHistory(userIdFor(req),limit):await getSessionHistory(userIdFor(req),sessionIdFor(req),limit)})}catch(error){res.status(500).json({ok:false,error:"Session load failed"})}});
