...
import { getObjectiveReliabilityReport, listScheduledObjectives } from "./objective-worker.js";
...
router.get("/activity",async(req,res)=>{try{const userId=userIdFor(req),[scorecard,objectives,approvals]=await Promise.all([getObjectiveReliabilityReport(userId),listScheduledObjectives(userId,{status:"all",limit:100}),listApprovals(userId,{status:"pending",limit:50})]);res.setHeader("Cache-Control","no-store");res.json({ok:true,scorecard,approvals:approvals.map(item=>({id:item.id,title:item.title||item.summary||"Approval needed",createdAt:item.createdAt||item.requestedAt||null})),objectives:objectives.map(item=>({id:item.id,title:item.title,status:item.status,stepIndex:item.stepIndex||0,stepCount:item.steps?.length||0,updatedAt:item.updatedAt||null,lastError:item.checkpoint?.lastError||null,recovered:(item.recoveryTrail?.length||0)>0,evidenceCount:(item.evidence||[]).filter(row=>row?.state==="verified").length}))});}catch(error){res.status(500).json({ok:false,error:error instanceof Error?error.message:"Activity unavailable"})}});
