...
export async function askGeorgie(input, history = [], context = "") {
  ...
  const safeHistory = Array.isArray(history) ? history.slice(-16).filter((item) => item && ["user", "assistant"].includes(item.role) && typeof item.content === "string") : [];
  const instructions = context ? `${SYSTEM_PROMPT}\n\nCURRENT CONTEXT\n${context}` : SYSTEM_PROMPT;
  const normalModel = process.env.OPENAI_MODEL || "gpt-5.6";
  const model = policy.reasoningEffort === "low" && process.env.OPENAI_FAST_MODEL ? process.env.OPENAI_FAST_MODEL : normalModel;
