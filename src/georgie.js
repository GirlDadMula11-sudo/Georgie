...
function reasoning(effort = "medium") {
  return { effort, context: "all_turns" };
}

async function jsonResponse({ model, instructions, input, effort = "medium" }) {
  const response = await openAI("/responses", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ model, instructions, input, reasoning: reasoning(effort), text: { verbosity: "low" } })
  });
...
export async function askGeorgie(input, history = [], context = "") {
  ...
  const safeHistory = Array.isArray(history)
    ? history.slice(-24).filter((item) => item && ["user", "assistant"].includes(item.role) && typeof item.content === "string")
    : [];
  ...
  const body = {
    ...
    reasoning: reasoning(process.env.OPENAI_REASONING_EFFORT || "medium"),
  ...
  return { text, responseId: payload.id, webSearches, model: body.model };
}
...
export async function analyzeOperationalEmail(message) {
  try {
    return await jsonResponse({
      ...
      input: JSON.stringify({ subject: message?.subject || "", from: message?.from || "", to: message?.to || "", date: message?.date || null, text: String(message?.text || "").slice(0, 16000) })
    });
  } catch {
...
export async function extractMemoryCandidates(userText, assistantText = "") {
  ...
  try {
    ...
    return parsed.memories.filter((item) => item && typeof item.text === "string" && item.text.trim()).slice(0, 5).map((item) => ({
      text: item.text.trim().slice(0, 2000), category: String(item.category || "fact").slice(0, 50), importance: Math.max(0, Math.min(1, Number(item.importance) || 0.5)), tags: Array.isArray(item.tags) ? item.tags.map(String).slice(0, 12) : []
    }));
  } catch { return []; }
}
...
export async function synthesizeSpeech(text) {
  ...
  const response = await openAI("/audio/speech", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ model: process.env.OPENAI_TTS_MODEL || "gpt-4o-mini-tts", voice: process.env.OPENAI_VOICE || "cedar", input: text.slice(0, 4096), response_format: "mp3", instructions: "Speak naturally, confidently, warmly, and efficiently. Sound like a highly capable executive and personal assistant." })
  });
