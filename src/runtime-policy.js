const TOOL_INTENT = /\b(open|launch|start|show|read|check|inspect|diagnose|health|status|deal|portfolio|lender|offer|pipeline|refresh|task|remind|email|mail|send|reply|search|find|clipboard|screen|screenshot|type|press|calendar|strategy|network|fix|repair|complete|finish|execute|verify|reconcile|stabilize|operate|functioning|prioritize)\b/i;
const CURRENT_INFO = /\b(today|tonight|current|currently|latest|recent|right now|news|weather|price|quote|yield|stock|market|crypto|bitcoin|ethereum|token|earnings|filing|dividend|portfolio|score|schedule|law|rule|regulation|availability|technology climate|tech climate|technology trend|tech trend|future of|future direction|state of ai|ai landscape|competitive landscape|emerging technology|frontier model|realtime ai|agentic|agents)\b/i;
...
export function runtimePolicy(input = "") {
  ...
  const balanced = BALANCED_REASONING.test(text);
  ...
  return {
    ...
    reasoningEffort: simple ? "low" : deep ? "high" : balanced ? "medium" : quick ? "low" : "medium",
    responseVerbosity: deep ? "medium" : words <= 12 ? "low" : "medium",
    latencyClass: simple ? "instant" : deep ? "deep" : balanced ? "standard" : quick ? "instant" : "standard"
  };
}
