...
import { orchestrateCapabilityRequest } from "./capability-orchestrator.js";
...
const CAPABILITIES = Object.freeze({
  ...
  "primary_mac.agent.maintenance": Object.freeze({
    targetDevice: "primary-mac",
    authority: "local_admin",
    operations: new Set(["update_restart_from_main", "install_neo_preload", "inspect_neo_preload", "normalize_generated_lock", "normalize_installer_generated_drift", "apply_neo_manifest_fix", "apply_governed_browser_agent", "apply_seo_autopilot_agent", "apply_seo_autopilot_agent_v2", "apply_seo_json_boundary"]),
...
export function validateCommandEnvelope(input = {}) {
  ...
  let capability = clean(input.capability || metadata.capability || metadata.requiredCapability || nested.capability, 160).toLowerCase();
  let targetDevice = clean(input.target_device || input.targetDevice || metadata.target_device || metadata.targetDevice || metadata.deviceId || nested.target_device || nested.targetDevice, 160);
  let operation = clean(input.operation || metadata.operation || nested.operation, 160).toLowerCase();
  let authority = clean(input.authority || metadata.authority || metadata.mode || nested.authority, 80).toLowerCase();
  let prohibitedRoutes = [...new Set((input.prohibited_routes || metadata.prohibited_routes || metadata.prohibitedRoutes || nested.prohibited_routes || nested.prohibitedRoutes || []).map((value) => clean(value, 160).toLowerCase()).filter(Boolean))];
...
async function executeTypedCapability({ userId, command }) {
  ...
  if (route.capability.startsWith("developer.")) {
    ...
    const existingJobId = clean(command.result?.job?.id || command.metadata?.existing_job_id || command.metadata?.existingJobId, 200);
  ...
  if (route.capability === "primary_mac.ui_keyboard") {
    ...
    if (route.operation === "focus_terminal") {
      const sequence = [
        ...
        { action: "ui.type_text", args: { text: "Terminal" }, suffix: "terminal" },
        { action: "ui.key", args: { key: "return", modifiers: [] }, suffix: "open" }
      ];
      const jobs = [];
      for (const item of sequence) {
        const job = await enqueueMacJob({ userId, deviceId: route.target_device, action: item.action, args: item.args, risk: "sensitive_write", reason: "Focus the authorized Mac Terminal through an allowlisted keyboard sequence", idempotencyKey: `connector:${command.id}:focus_terminal:${item.suffix}`, maxAttempts: 1 });
        jobs.push({ id: job.id, status: job.status, action: item.action, deviceId: route.target_device, dispatchReceipt: job.dispatchReceipt });
      }
      return { terminalState: "in_progress", completed: false, route, jobs, productionMutation: false };
    }
  ...
  if (route.capability === "primary_mac.agent.maintenance") {
    ...
    const installerGeneratedDriftPatch = `diff --git a/mac-agent/agent.js b/mac-agent/agent.js
--- a/mac-agent/agent.js
+++ b/mac-agent/agent.js
@@ -11,19 +11,10 @@ import { verifyNeoCdpSession } from "./neo-cdp-reader.js";
...
@@ -665,7 +656,6 @@ async function cycle() {
...
`;
    ...
    const specs = route.operation === "apply_seo_json_boundary"
      ...
      : route.operation === "normalize_installer_generated_drift"
        ? [["developer.apply_patch", { repo, patch: installerGeneratedDriftPatch, patchHash: digest(installerGeneratedDriftPatch) }, "Normalize the hash-proven daemon-health and package-lock installer drift"]]
...
export function createGovernedConnector({ executeCommand, emitStatus = async () => {}, readState, writeState, retainObjective, transitionObjective, leaseTtlMs = 120_000, recoveryBaseDelayMs = 1_000, recoveryMaxDelayMs = 60_000, recoveryMaxAttempts = 8, ownerId = null } = {}) {
  ...
  const boundedRecoveryBaseDelayMs = Math.max(10, Math.min(60_000, Number(recoveryBaseDelayMs) || 1_000));
  const boundedRecoveryMaxDelayMs = Math.max(boundedRecoveryBaseDelayMs, Math.min(900_000, Number(recoveryMaxDelayMs) || 60_000));
  const boundedRecoveryMaxAttempts = Math.max(1, Math.min(100, Number(recoveryMaxAttempts) || 8));
  ...
  async function record(userId, command, status, payload = {}, claim = null) {
    const recorded = await exclusive(userId, async () => {
      ...
      if (item) {
        item.status = status;
        item.updatedAt = now();
        if (["completed", "blocked"].includes(status)) item.completedAt = item.updatedAt;
        if (["failed", "recovering", "blocked"].includes(status)) item.error = clean(payload.error, 1000);
        if (payload.resultSummary) item.result = payload.resultSummary;
      }
      ...
      if (lease && claim) {
        lease.status = status === "completed" ? "completed" : status === "blocked" ? "blocked" : status === "failed" ? "failed" : status === "recovering" ? "queued" : status;
        lease.terminalReceiptId = ["completed", "blocked", "failed"].includes(status) ? receipt.receiptId : lease.terminalReceiptId;
        lease.updatedAt = event.createdAt;
        if (lease.status === "queued") {
          lease.owner = null;
          lease.expiresAt = new Date(Date.now() + boundedLeaseTtlMs).toISOString();
        }
      }
    ...
    setImmediate(() => emitStatus({ ...recorded.event, receipt: recorded.receipt }).catch(() => {}));
    return recorded.receipt;
  }
  async function ensureOperatingNode(userId, command) {
    if (command.operatingNodeId) return command.operatingNodeId;
    const node = await retain(userId, { stableKey: `connector:${command.objectiveId}`, kind: command.kind === "approval" ? "execution" : "objective", title: command.command.slice(0, 240), domain: "general", status: "planned", nextAction: "Dispatch through the governed connector and preserve completion evidence.", approvalId: command.approvalId });
    command.operatingNodeId = node.id;
    await exclusive(userId, async () => {
      const state = await read(userId);
      const item = state.commands.find(row => row.id === command.id);
      if (item && !item.operatingNodeId) item.operatingNodeId = node.id;
      await persist(userId, state);
    });
    return node.id;
  }
  async function run(userId, command) { const claimResult=await acquireOrReturnLease(userId,command); if(claimResult.terminal||!claimResult.acquired)return{commandId:command.id,objectiveId:command.objectiveId,status:claimResult.lease?.status||command.status,lease:claimResult.lease,duplicateExecutionPrevented:true}; const claim=claimResult.lease; await record(userId,command,"running",{},claim); const operatingNodeId=await ensureOperatingNode(userId,command).catch(()=>null); if(operatingNodeId)await transition(userId,operatingNodeId,{status:"active",attempted:true,nextAction:"Execute, verify, and return durable evidence."}).catch(()=>{}); const heartbeat=setInterval(()=>heartbeatLease(userId,claim).catch(()=>{}),Math.max(500,Math.floor(boundedLeaseTtlMs/3)));heartbeat.unref?.(); try{ const result=command.routing?await executeTypedCapability({userId,command}):await executeCommand({userId,sessionId:`connector:${command.source}:objective:${command.objectiveId}`,input:command.command,connector:{commandId:command.id,objectiveId:command.objectiveId,planId:command.planId,approvalId:command.approvalId,leaseId:claim.id,leaseGeneration:claim.generation}}); const resultSummary=command.routing?{terminalState:result?.terminalState||null,completed:result?.completed===true,route:result?.route||null,job:result?.job||null,jobs:result?.jobs||null,evidence:Array.isArray(result?.evidence)?result.evidence.slice(0,50):[],errors:Array.isArray(result?.errors)?result.errors.slice(0,50):[],cursors:result?.cursors||{},mailboxMutation:result?.mailboxMutation===true,markSeen:result?.markSeen===true,prohibitedTool:result?.prohibitedTool||null,expectedAgentVersion:result?.expectedAgentVersion||null,projection:result?.projection||null,integration:result?.integration||null,websiteControl:result?.websiteControl||null,crawl:result?.crawl?{...result.crawl,pages:Array.isArray(result.crawl.pages)?result.crawl.pages.slice(0,150):[]}:null,performance:Array.isArray(result?.performance)?result.performance.slice(0,20):[],applicationFunnel:result?.applicationFunnel||null,applicationFunnelError:result?.applicationFunnelError||null,defects:result?.defects||null,productionMutation:result?.productionMutation===true,scheduledObjective:result?.scheduledObjective||null,objectiveStatus:result?.objectiveStatus||null,repositoryInspection:result?.repositoryInspection||null,sourceInspection:result?.sourceInspection||null,inspectionResultReturned:result?.inspectionResultReturned===true,liveManifestVerification:result?.liveManifestVerification||null,liveManifestVerified:result?.liveManifestVerification?.verified===true}:{text:clean(result?.text||result?.response||"",50000),actions:Array.isArray(result?.actions)?result.actions.slice(0,100):[]}; const terminalState=clean(result?.outcome?.terminalState||result?.terminalState||(result?.completed===false?"recovering":"completed"),80),evidence={responseHash:digest(JSON.stringify(result||{})),terminalState,...(resultSummary?{resultSummary}:{})}; if(result?.completed===false||["in_progress","working","recovering","queued","running"].includes(terminalState)){const exhausted=Number(claim.attempts||0)>=boundedRecoveryMaxAttempts,status=exhausted?"blocked":"recovering",error=clean(result?.error||result?.exactBlocker||terminalState,1000);const receipt=await record(userId,command,status,{...evidence,error},claim);await transition(userId,command.operatingNodeId,exhausted?{status:"blocked",nextAction:`Automatic recovery exhausted after ${claim.attempts} attempts: ${error}`}:{status:"recovering",recovery:"Resume this same command and lease checkpoint; do not create a duplicate.",nextAction:"Continue from the durable lease checkpoint."}).catch(()=>{});return{commandId:command.id,objectiveId:command.objectiveId,status,result,receipt,lease:await readLease(userId,command.id),recoveryExhausted:exhausted};} const failedActions=Array.isArray(result?.actions)&&result.actions.length>0&&result.actions.every(action=>action?.ok===false); const blocked=terminalState==="blocked"||result?.outcome?.terminalState==="blocked"||failedActions; const receipt=await record(userId,command,blocked?"blocked":"completed",evidence,claim);await transition(userId,command.operatingNodeId,blocked?{status:"blocked",nextAction:clean(result?.exactBlocker||result?.error||"Resolve the verified blocker and resume the same objective.",1000)}:{status:"verified",verification:`Connector completion receipt ${receipt.receiptId}`}).catch(()=>{});return{commandId:command.id,objectiveId:command.objectiveId,status:blocked?"blocked":"completed",result,receipt,lease:await readLease(userId,command.id)}; }catch(error){const message=error instanceof Error?error.message:String(error);if(/^LEASE_FENCED:/.test(message))return{commandId:command.id,objectiveId:command.objectiveId,status:(await readLease(userId,command.id))?.status||"running",error:message,lease:await readLease(userId,command.id),duplicateExecutionPrevented:true};const exhausted=Number(claim.attempts||0)>=boundedRecoveryMaxAttempts,status=exhausted?"blocked":"recovering";const receipt=await record(userId,command,status,{error:message},claim);await transition(userId,command.operatingNodeId,exhausted?{status:"blocked",nextAction:`Automatic recovery exhausted after ${claim.attempts} attempts: ${message}`}:{status:"recovering",recovery:"Resume this same command ID after the temporary blocker is resolved; do not create a duplicate.",nextAction:message}).catch(()=>{});return{commandId:command.id,objectiveId:command.objectiveId,status,error:message,receipt,lease:await readLease(userId,command.id),recoveryExhausted:exhausted};}finally{clearInterval(heartbeat);} }
  const scheduledRuns = new Set();
  const recoveryTimers = new Map();
  function recoveryDelay(attempts=1){return Math.min(boundedRecoveryMaxDelayMs,boundedRecoveryBaseDelayMs*(2**Math.max(0,Math.min(10,Number(attempts||1)-1))));}
  function scheduleRecovery(userId,command,lease){
    const key=`${String(userId)}:${command.id}`;
    if(recoveryTimers.has(key)||terminalLease(lease))return false;
    const timer=setTimeout(()=>{recoveryTimers.delete(key);schedule(userId,command);},recoveryDelay(lease?.attempts));
    timer.unref?.();
  ...
  function schedule(userId,command){
    ...
    setImmediate(async()=>{
      let outcome=null;
      try{outcome=await run(userId,command);}
        ...
        scheduledRuns.delete(key);
        if(outcome?.status==="recovering")scheduleRecovery(userId,command,outcome.lease);
      }
    });
    return true;
  }
  // SEO_PHASE2_TRANSPORT_REOPEN: scheduling is nonterminal until the durable semantic objective verifies.
  async function reopenPhase2TransportIfNeeded(userId,command){const phase2CommandId=clean(command?.metadata?.phase2_command_id||command?.metadata?.phase2CommandId||command?.metadata?.command_id||command?.metadata?.commandId,220);if(command?.routing?.capability!=="sierra.seo.autopilot"||command?.status!=="completed"||!phase2CommandId)return false;const objectives=(await listScheduledObjectives(userId,{status:"all",limit:100})).filter(item=>item.stableKey===`seo-phase2:${phase2CommandId}`);const current=objectives.sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt)))[0]||null;if(current?.status==="verified")return false;return exclusive(userId,async()=>{const state=await read(userId),item=state.commands.find(row=>row.id===command.id),phaseLease=leaseFor(state,command.id);if(!item||item.status!=="completed")return false;item.status="recovering";item.completedAt=null;item.error=clean(current?.checkpoint?.lastError||"SEO_PHASE2_SEMANTIC_VERIFICATION_PENDING",1000);item.updatedAt=now();if(phaseLease){phaseLease.status="queued";phaseLease.owner=null;phaseLease.terminalReceiptId=null;phaseLease.expiresAt=now();phaseLease.updatedAt=now();}await persist(userId,state);return true;});}
  async function submit(userId="primary",input={}){
    const envelope=validateCommandEnvelope(input);
    let command,duplicate=false,acceptedReceipt,acceptedEvent,lease;
    await exclusive(userId,async()=>{
      ...
      acceptedEvent={id:crypto.randomUUID(),commandId:id,objectiveId:objective,status:"accepted",createdAt:now()};
      acceptedReceipt=receiptFor(command,"accepted",{leaseId:lease.id});
      state.events.push(acceptedEvent);
      state.receipts.push(acceptedReceipt);
      await persist(userId,state);
    });
    if(duplicate){
      const phase2Reopened=await reopenPhase2TransportIfNeeded(userId,command);
      if(phase2Reopened){command.status="recovering";lease=await readLease(userId,command.id);}
      if(["accepted","running","recovering","failed"].includes(command.status)&&(!activeLease(lease)||lease?.status==="queued"))schedule(userId,command);
      return{commandId:command.id,objectiveId:command.objectiveId,status:command.status,duplicate:true,lease:leasePublic(lease),result:command.result||null};
    }
    setImmediate(()=>emitStatus({...acceptedEvent,receipt:acceptedReceipt}).catch(()=>{}));
    schedule(userId,command);
    return{commandId:command.id,objectiveId:command.objectiveId,status:"accepted",lease:leasePublic(lease),receipt:acceptedReceipt};
  }
  async function status(userId="primary",id){
    const state=await read(userId),command=state.commands.find(row=>row.id===id);
    if(!command)return null;
    const response={...command,lease:leasePublic(leaseFor(state,id)),events:state.events.filter(row=>row.commandId===id),receipts:state.receipts.filter(row=>row.commandId===id),statusReadback:{core:"available",enrichment:"available",degradedProviders:[]}};
    try{
      if(command.routing?.capability==="primary_mac.agent.maintenance"){
        ...
        const expectedAgentVersion=clean(command.result?.expectedAgentVersion,50)||null;
        const primaryDevice=response.macDevices.find(device=>device.deviceId==="primary-mac");
        const verifiedDevice=expectedAgentVersion&&primaryDevice?.online===true&&primaryDevice.agentVersion===expectedAgentVersion?primaryDevice:null;
        response.macInstallVerification={expectedAgentVersion,verified:Boolean(verifiedDevice),deviceId:"primary-mac",observedAgentVersion:primaryDevice?.agentVersion||null,lastSeenAt:primaryDevice?.lastSeenAt||null};
      }
      ...
      if(jobId&&command.objectiveId){
        const developerJob=command.routing?.capability?.startsWith("developer.");
        const job=(await listMacJobs(userId,500)).find(item=>item.id===jobId&&(developerJob||String(item.args?.objectiveId||"")===command.objectiveId));
      ...
      response.statusReadback.enrichment="degraded";
      response.statusReadback.degradedProviders.push({provider:"optional_command_enrichment",error:clean(error instanceof Error?error.message:String(error),500)});
    }
    return response;
  }
...
export function createGovernedConnectorRouter({ executeCommand, emitStatus } = {}) {
  ...
  const recoverySweepMs=Math.max(1_000,Math.min(300_000,Number(process.env.GEORGIE_CONNECTOR_RECOVERY_SWEEP_MS)||15_000));
  const recoverySweep=setInterval(()=>connector.resume("primary").catch(error=>console.warn("[Georgie] connector recovery sweep deferred:",error instanceof Error?error.message:error)),recoverySweepMs);
  recoverySweep.unref?.();
  ...
  router.get("/commands/:id", async (req, res) => { try{const result = await connector.status(req.query?.userId || "primary", req.params.id);res.status(result ? 200 : 404).json(result ? { ok: true, command: result } : { ok: false, error: "Command not found" });}catch(error){res.status(503).json({ok:false,error:"COMMAND_STATUS_TEMPORARILY_UNAVAILABLE",retryable:true,commandId:req.params.id,statusReadback:{core:"degraded",enrichment:"unavailable"},detail:clean(error instanceof Error?error.message:String(error),500)});} });
  return router;
}
