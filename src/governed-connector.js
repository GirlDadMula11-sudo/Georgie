...
import { listRecentMessages, readMessage } from "./integrations/neo-mail.js";
...
async function executeTypedCapability({ userId, command }) {
  ...
  if (route.capability === "neo_mail.imap.read_only") {
    ...
    return { terminalState: evidence.length ? "completed" : "blocked", completed: evidence.length > 0, route, evidence, errors, mailboxMutation: false, markSeen: false, prohibitedTool: "email.send" };
  }
