...
export function summarizeGovernedMacJob(job = {}) { const repositoryInspection=job.action==="developer.repo_inspect"&&job.result?{repo:job.result.repo||null,branch:job.result.branch||null,status:job.result.status||"",recentCommits:job.result.recentCommits||"",readOnly:job.result.readOnly===true}:null; return { id: job.id, status: job.status, action: job.action, deviceId: job.deviceId, authority: job.args?.authority || null, checkpoint: job.args?.checkpoint || null, attempts: job.attempts, claimedAt: job.claimedAt, completedAt: job.completedAt, error: job.error, dispatchReceipt: job.dispatchReceipt, cursor: job.result?.mailboxEvidenceBatch?.cursor || {}, packetCount: job.result?.mailboxEvidenceBatch?.packets?.length || 0, quarantineCount: job.result?.quarantine?.length || job.result?.mailboxEvidenceBatch?.quarantine?.length || 0, connections: job.result?.connection || null, staticContractInspection: job.result?.neoStaticContractInspection || null, browserInspection: job.result?.governedBrowserInspection || null, repositoryInspection }; }
