...
export function createGovernedConnector({ executeCommand, emitStatus = async () => {}, readState, writeState, retainObjective, transitionObjective, leaseTtlMs = 30_000, ownerId = null } = {}) {
  ...
  async function status(userId="primary",id){const state=await read(userId),command=state.commands.find(row=>row.id===id);if(!command)return null;const response={...command,lease:leasePublic(leaseFor(state,id)),events:state.events.filter(row=>row.commandId===id),receipts:state.receipts.filter(row=>row.commandId===id)};if(command.routing?.capability==="primary_mac.agent.maintenance"){const ids=new Set((command.result?.jobs||[]).map(job=>job.id));response.macJobs=(await listMacJobs(userId,500)).filter(job=>ids.has(job.id)).map(job=>({id:job.id,status:job.status,action:job.action,deviceId:job.deviceId,attempts:job.attempts,claimedAt:job.claimedAt,completedAt:job.completedAt,error:job.error,dispatchReceipt:job.dispatchReceipt}));response.macDevices=getMacDeviceStatus();const expectedAgentVersion=clean(command.result?.expectedAgentVersion,50)||null;const primaryDevice=response.macDevices.find(device=>device.deviceId==="primary-mac");const verifiedDevice=expectedAgentVersion&&primaryDevice?.online===true&&primaryDevice.agentVersion===expectedAgentVersion?primaryDevice:null;response.macInstallVerification={expectedAgentVersion,verified:Boolean(verifiedDevice),deviceId:"primary-mac",observedAgentVersion:primaryDevice?.agentVersion||null,lastSeenAt:primaryDevice?.lastSeenAt||null};}const jobId=clean(command.result?.job?.id||command.metadata?.existing_job_id||command.metadata?.existingJobId,200);if(jobId&&command.objectiveId){const developerJob=command.routing?.capability?.startsWith("developer.");const job=(await listMacJobs(userId,500)).find(item=>item.id===jobId&&(developerJob||String(item.args?.objectiveId||"")===command.objectiveId));if(job)response.macJob=summarizeGovernedMacJob(job);response.packetManifests=await listMailboxPacketManifests(userId,{objectiveId:command.objectiveId,limit:25});}return response;}
