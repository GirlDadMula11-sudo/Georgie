...
export async function registerParticipant(userId=USER(),input={}){
  ...
  state.participants[id]={...existing,id,role:bounded(input.role||existing.role,120),capabilities:Array.isArray(input.capabilities)?[...new Set(input.capabilities.map(v=>bounded(v,160)).filter(Boolean))].slice(0,200):existing.capabilities||[],callbackMode:bounded(input.callbackMode||existing.callbackMode||"pull",80),endpointBound:Boolean(input.endpointBound),lastSeenAt:now(),updatedAt:now()};
  await save(userId,state);return clone(state.participants[id]);
}

export async function ensureObjective(userId=USER(),input={}){
...
export async function appendEvidence(userId=USER(),input={}){
...
export async function acquireLock(userId=USER(),{objectiveId,owner,resource,ttlMs=DEFAULT_LEASE_MS}={}){
...
export async function releaseLock(userId=USER(),{resource,owner}={}){
...
export async function issueCommand(userId=USER(),input={}){
...
export async function createHandoff(userId=USER(),input={}){
...
export async function acknowledgeHandoff(userId=USER(),{handoffId,participant}={}){
...
export async function recordCallback(userId=USER(),input={}){
...
export async function controlPlaneSnapshot(userId=USER(),{objectiveId=null}={}){
  ...
  return{version:state.version,mission:SHARED_MISSION,participants:state.participants,objectives:oid?state.objectives.filter(row=>row.id===oid):state.objectives.slice(-100),evidence:(oid?state.evidence.filter(row=>row.objectiveId===oid):state.evidence.slice(-300)),commands:(oid?state.commands.filter(row=>row.objectiveId===oid):state.commands.slice(-200)),handoffs:(oid?state.handoffs.filter(row=>row.objectiveId===oid):state.handoffs.slice(-200)),callbacks:(oid?state.callbacks.filter(row=>row.objectiveId===oid):state.callbacks.slice(-200)),locks:state.locks.filter(lock=>Date.parse(lock.expiresAt||0)>Date.now()),updatedAt:state.updatedAt};
}

export async function prepareObjectiveControlContext(userId,input={}){
  const objective=await ensureObjective(userId,input);
  await Promise.all([
    registerParticipant(userId,{...PARTICIPANTS.georgie,capabilities:["persistent_execution","background_recovery","governed_tools","evidence_persistence"],callbackMode:"durable_pull",endpointBound:true}),
    registerParticipant(userId,{...PARTICIPANTS.chatgpt,capabilities:["interactive_reasoning","architecture","connected_tool_coordination","code_review"],callbackMode:"conversation_only",endpointBound:false})
  ]);
  const snapshot=await controlPlaneSnapshot(userId,{objectiveId:objective.id});
  return{objectiveId:objective.id,missionId:SHARED_MISSION.id,objective,snapshot,connectionTruth:{chatgptAutonomousCallback:false,chatgptConversationRequired:true,openaiApiPeerAvailableOnlyWhenConfigured:true,georgiePersistent:true}};
}
