...
export async function claimMacJobs(deviceId,limit=5){const uid=safeUserId(PRIMARY());const store=await readStore(uid);const jobs=store.jobs.filter(j=>j.deviceId===deviceId&&j.status==="queued").slice(0,limit);const now=new Date().toISOString();for(const job of jobs){job.status="claimed";job.claimedAt=now}if(jobs.length)await writeStore(uid,store);return jobs}
