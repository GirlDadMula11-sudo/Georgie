...
export async function enqueueMacJob({ userId, deviceId, action, args = {}, risk = "low_risk_write", reason = "" }) {
  ...
  const job = {
    id: crypto.randomUUID(), userId, deviceId, action, args, risk, reason,
...
export async function claimMacJobs(deviceId, limit = 5) {
  ...
  if (jobs.length) await writeStore(store);
  return jobs;
}
...
export async function listMacJobs(userId, limit = 50) {
  const store = await readStore();
  return store.jobs.filter(j => j.userId === userId).sort((a,b) => String(b.createdAt).localeCompare(String(a.createdAt))).slice(0, limit);
}
