...
const LONG_RUNNING_MAC_ACTIONS=new Set(["roblox.install_rojo_and_build"]);
...
export async function recoverLongRunningMacJob(deviceId,jobId,{expectedAction,requiredAgentVersion}={}){const uid=safeUserId(PRIMARY());return mutateStore(uid,store=>{const job=store.jobs.find(item=>item.id===String(jobId)&&item.deviceId===String(deviceId));if(!job)return null;const action=String(expectedAction||"");if(!LONG_RUNNING_MAC_ACTIONS.has(action)||job.action!==action)throw new Error("MAC_LONG_RUNNING_RECOVERY_SCOPE_REJECTED");if(!/^2\.2\.\d+$/.test(String(requiredAgentVersion||"")))throw new Error("MAC_LONG_RUNNING_RECOVERY_VERSION_INVALID");if(job.status==="completed")return job;if(job.status==="claimed"&&job.claimLeaseExpiresAt&&new Date(job.claimLeaseExpiresAt).getTime()>Date.now())throw new Error("MAC_LONG_RUNNING_JOB_STILL_ACTIVE");if(!["queued","claimed","failed","dead_letter"].includes(job.status))throw new Error(`MAC_LONG_RUNNING_JOB_NOT_RECOVERABLE: ${job.status}`);const recoveredAt=new Date().toISOString();job.resumeHistory=[...(job.resumeHistory||[]),{resumedAt:recoveredAt,fromStatus:job.status,error:job.error,attempts:job.attempts,reason:"long_running_checkpoint_transport_repaired"}].slice(-20);job.args={...structuredClone(job.args||{}),requiredAgentVersion:String(requiredAgentVersion)};job.status="queued";job.attempts=0;job.maxAttempts=Math.max(1,Math.min(5,Number(job.maxAttempts)||5));job.availableAt=recoveredAt;job.claimedAt=null;job.claimLeaseExpiresAt=null;job.completedAt=null;job.result=null;job.error=null;job.alert=null;job.resumeCount=Number(job.resumeCount||0)+1;return job;},{durableClaimBoundary:true});}
