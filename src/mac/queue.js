...
function compactJobStore(store){
  const jobs=Array.isArray(store?.jobs)?store.jobs.slice(-500):[];
...
export async function enqueueMacJob({userId,deviceId,action,args={},risk="low_risk_write",reason="",idempotencyKey=null,approvalId=null,planId=null,maxAttempts=5}){
  ...
  return mutateStore(queueUserId,store=>{
    ...
    const job={id:jobId,userId:queueUserId,requestedByUserId,deviceId,action,args,risk,reason,idempotencyKey:key,approvalId,planId,status:"queued",attempts:0,maxAttempts:Math.max(1,Math.min(5,Number(maxAttempts)||5)),createdAt:acceptedAt,availableAt:acceptedAt,claimedAt:null,claimLeaseExpiresAt:null,completedAt:null,result:null,error:null,dispatchReceipt:{id:crypto.randomUUID(),jobId,idempotencyKey:key,acceptedAt,claimedAt:null,deviceId:null},alert:null};
...
export async function importRecoveredMacJob(input){
  ...
  return mutateStore(uid,store=>{const existing=store.jobs.find(item=>item.id===job.id);if(existing){if(JSON.stringify(existing)!==JSON.stringify(job))throw new Error("MAC_RECOVERY_IMPORT_CONFLICT");return existing;}store.jobs.push(job);store.jobs=store.jobs.slice(-5000);return job;});
}
...
export async function recoverLongRunningMacJob(deviceId,jobId,{expectedAction,requiredAgentVersion}={}){const uid=safeUserId(PRIMARY());return mutateStore(uid,store=>{const job=store.jobs.find(item=>item.id===String(jobId)&&item.deviceId===String(deviceId));if(!job)return null;const action=String(expectedAction||"");if(!LONG_RUNNING_MAC_ACTIONS.has(action)||job.action!==action)throw new Error("MAC_LONG_RUNNING_RECOVERY_SCOPE_REJECTED");if(!/^2\.2\.\d+$/.test(String(requiredAgentVersion||"")))throw new Error("MAC_LONG_RUNNING_RECOVERY_VERSION_INVALID");const markerOnlyPlayTestFailure=action==="roblox.play_test_validate"&&job.status==="completed"&&job.result?.status==="blocked"&&Array.isArray(job.result?.defects)&&job.result.defects.length===1&&job.result.defects[0]==="RUNTIME_PROTOTYPE_MARKER_NOT_OBSERVED";const screenshotOnlyPlayTestFailure=action==="roblox.play_test_validate"&&job.status==="failed"&&/Command failed: screencapture -x /.test(String(job.error||""));if(job.status==="completed"&&!markerOnlyPlayTestFailure)return job;if(job.status==="claimed"&&job.claimLeaseExpiresAt&&new Date(job.claimLeaseExpiresAt).getTime()>Date.now())throw new Error("MAC_LONG_RUNNING_JOB_STILL_ACTIVE");if(!["queued","claimed","failed","dead_letter","completed"].includes(job.status))throw new Error(`MAC_LONG_RUNNING_JOB_NOT_RECOVERABLE: ${job.status}`);const recoveredAt=new Date().toISOString();job.resumeHistory=[...(job.resumeHistory||[]),{resumedAt:recoveredAt,fromStatus:job.status,error:job.error,attempts:job.attempts,reason:markerOnlyPlayTestFailure?"play_test_studio_window_readiness_repaired":screenshotOnlyPlayTestFailure?"play_test_screenshot_evidence_repaired":"long_running_checkpoint_transport_repaired"}].slice(-20);job.args={...structuredClone(job.args||{}),requiredAgentVersion:String(requiredAgentVersion)};job.status="queued";job.attempts=0;job.maxAttempts=Math.max(1,Math.min(5,Number(job.maxAttempts)||5));job.availableAt=recoveredAt;job.claimedAt=null;job.claimLeaseExpiresAt=null;job.completedAt=null;job.result=null;job.error=null;job.alert=null;job.resumeCount=Number(job.resumeCount||0)+1;return job;},{durableClaimBoundary:true});}
