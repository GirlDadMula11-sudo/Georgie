...
export async function resumeFailedMacJob(deviceId,jobId,{objectiveId,expectedAction}={}){const uid=safeUserId(PRIMARY());return mutateStore(uid,store=>{const job=store.jobs.find(j=>j.id===jobId&&j.deviceId===deviceId);if(!job)return null;if(!["failed","dead_letter"].includes(job.status))throw new Error(`MAC_JOB_NOT_RESUMABLE: ${job.status}`);if(String(job.args?.objectiveId||"")!==String(objectiveId||""))throw new Error("MAC_JOB_OBJECTIVE_MISMATCH");if(job.action!==String(expectedAction||""))throw new Error("MAC_JOB_ACTION_MISMATCH");if(job.action!=="mailbox.read_only_backfill"||job.risk!=="read"||job.args?.authority!=="read_only")throw new Error("MAC_JOB_RESUME_SCOPE_REJECTED");if(!/^Unsupported Mac action: mailbox\.read_only_backfill$/.test(String(job.error||"")))throw new Error("MAC_JOB_FAILURE_NOT_VERSION_RECOVERABLE");const resumedAt=new Date().toISOString();job.resumeHistory=[...(Array.isArray(job.resumeHistory)?job.resumeHistory:[]),{resumedAt,fromStatus:job.status,error:job.error,attempts:job.attempts,completedAt:job.completedAt,reason:"handler_version_repaired"}].slice(-20);job.status="queued";job.availableAt=resumedAt;job.claimedAt=null;job.claimLeaseExpiresAt=null;job.completedAt=null;job.result=null;job.error=null;job.alert=null;job.maxAttempts=Math.max(Number(job.maxAttempts||5),Number(job.attempts||0)+5);job.resumeCount=Number(job.resumeCount||0)+1;return job;});}
