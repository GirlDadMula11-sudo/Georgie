...
function versionRecoverableMailboxJob(job){if(["failed","dead_letter"].includes(job.status))return /^Unsupported Mac action: mailbox\.read_only_backfill$/.test(String(job.error||""))?"handler_version_repaired":null;if(job.status!=="completed")return null;const batch=job.result?.mailboxEvidenceBatch,connections=Object.values(job.result?.connection||{});const empty=Array.isArray(batch?.packets)&&batch.packets.length===0&&Object.keys(batch?.cursor||{}).length===0;const legacyMiss=connections.length>0&&connections.every(item=>item?.connected===false&&item?.error==="configured account not found");return empty&&legacyMiss?"legacy_reader_replaced":null;}
export async function resumeFailedMacJob(deviceId,jobId,{objectiveId,expectedAction}={}){const uid=safeUserId(PRIMARY());return mutateStore(uid,store=>{const job=store.jobs.find(j=>j.id===jobId&&j.deviceId===deviceId);if(!job)return null;if(String(job.args?.objectiveId||"")!==String(objectiveId||""))throw new Error("MAC_JOB_OBJECTIVE_MISMATCH");if(job.action!==String(expectedAction||""))throw new Error("MAC_JOB_ACTION_MISMATCH");if(job.action!=="mailbox.read_only_backfill"||job.risk!=="read"||job.args?.authority!=="read_only")throw new Error("MAC_JOB_RESUME_SCOPE_REJECTED");const reason=versionRecoverableMailboxJob(job);if(!reason)throw new Error(`MAC_JOB_NOT_RESUMABLE: ${job.status}`);const resumedAt=new Date().toISOString();job.resumeHistory=[...(Array.isArray(job.resumeHistory)?job.resumeHistory:[]),{resumedAt,fromStatus:job.status,error:job.error,attempts:job.attempts,completedAt:job.completedAt,resultHash:job.result?crypto.createHash("sha256").update(JSON.stringify(job.result)).digest("hex"):null,reason}].slice(-20);job.status="queued";job.availableAt=resumedAt;job.claimedAt=null;job.claimLeaseExpiresAt=null;job.completedAt=null;job.result=null;job.error=null;job.alert=null;job.maxAttempts=Math.max(Number(job.maxAttempts||5),Number(job.attempts||0)+5);job.resumeCount=Number(job.resumeCount||0)+1;return job;});}
export async function checkpointMacJob(deviceId,jobId,checkpoint){const uid=safeUserId(PRIMARY());return mutateStore(uid,store=>{const job=store.jobs.find(j=>j.id===jobId&&j.deviceId===deviceId&&j.status==="claimed");if(!job)return null;const next=Number(checkpoint?.nextStep)||0;if(next<Number(job.workflowCheckpoint?.nextStep||0))throw new Error("Workflow checkpoint cannot move backward");const prior=Array.isArray(job.workflowCheckpoint?.receipts)?job.workflowCheckpoint.receipts:[],receipt=checkpoint?.receipt||null;job.workflowCheckpoint={nextStep:next,stepId:String(checkpoint?.stepId||"").slice(0,120),receipt,receipts:receipt?[...prior.filter(item=>item?.stepId!==receipt.stepId),receipt].slice(-40):prior,updatedAt:new Date().toISOString()};return job;});}
