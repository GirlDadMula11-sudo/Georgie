...
export function deterministicToolPlan(input = "") {
  ...
  if (/\b(smartlead|campaigns?|deliverability)\b/.test(lower) && /\b(diagnos|repair|broken|not sending|low volume|why|problem|issue)\b/.test(lower)) return [{ tool: "campaigns.diagnose", args: {} }];
