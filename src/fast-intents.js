...
export function deterministicToolPlan(input = "") {
  ...
  if (longRunningMacRecoveryMarker) {
    ...
    if (!/^idem-[0-9a-f]{40}$/.test(jobId)
      ...
      || requiredAgentVersion !== "2.2.42") return [];
    return [{tool:"approvals.prepare_plan",args:{
      ...
      verificationMethod:"Require the same job ID, roblox.install_rojo_and_build action, queued or completed recovery state, agent 2.2.42 binding, and a fresh primary-mac heartbeat.",
  ...
  if (robloxPlayTestMarker) {
    ...
    const requiredAgentVersion = String(request?.requiredAgentVersion || "");
    if (projectRoot !== "/Users/mac/Documents/Georgie Roblox Projects/makayla-horror-prototype" || requiredAgentVersion !== "2.2.42") return [];
    return [{tool:"approvals.prepare_plan",args:{
      ...
      domain:"technical",risk:"high",reversible:true,
      verificationMethod:"Require agent 2.2.42, all six gameplay checks true, the runtime marker observed, play mode stopped, and an empty defect list.",
  ...
  const robloxPlayTestRecoveryMarker = text.match(/ROBLOX_PLAY_TEST_RECOVERY_JSON:\s*(\{[\s\S]*\})\s*$/i);
  if (robloxPlayTestRecoveryMarker) {
    let request;
    try { request = JSON.parse(robloxPlayTestRecoveryMarker[1]); } catch { return []; }
    const jobId = String(request?.jobId || "").toLowerCase();
    const deviceId = String(request?.deviceId || "");
    const expectedAction = String(request?.expectedAction || "");
    const projectRoot = String(request?.projectRoot || "");
    ...
    return [{tool:"approvals.prepare_plan",args:{
      ...
      summary:`Recover ${jobId} in place after the runtime-log scan and lease repair. Preserve its identity and do not enqueue another play-test or Roblox build job.`,
      steps:["Validate the exact completed marker-only defect receipt.","Requeue only the same durable play-test identity on agent 2.2.42.","Read back all six checks, runtime evidence, play-stop state, and defects."],
      domain:"technical",risk:"high",reversible:true,
