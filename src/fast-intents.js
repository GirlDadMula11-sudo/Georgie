...
export function deterministicToolPlan(input = "") {
  ...
  if (longRunningMacRecoveryMarker) {
    ...
    if (!/^idem-[0-9a-f]{40}$/.test(jobId)
      ...
      || requiredAgentVersion !== "2.2.49") return [];
    return [{tool:"approvals.prepare_plan",args:{
      ...
      verificationMethod:"Require the same job ID, roblox.install_rojo_and_build action, queued or completed recovery state, agent 2.2.49 binding, and a fresh primary-mac heartbeat.",
  ...
  if (robloxPlayTestMarker) {
    ...
    const requiredAgentVersion = String(request?.requiredAgentVersion || "");
    if (projectRoot !== "/Users/mac/Documents/Georgie Roblox Projects/makayla-horror-prototype" || requiredAgentVersion !== "2.2.49") return [];
    return [{tool:"approvals.prepare_plan",args:{
      ...
      domain:"technical",risk:"high",reversible:true,
      verificationMethod:"Require agent 2.2.49, an exact Prototype.rbxlx document-window match, all six gameplay checks true, the runtime marker observed, play mode stopped, and an empty defect list.",
  ...
  if (robloxPlayTestRecoveryMarker) {
    ...
    return [{tool:"approvals.prepare_plan",args:{
      ...
      steps:["Validate the exact blocked play-test receipt.","Requeue only the same durable play-test identity on agent 2.2.49.","Read back the exact Prototype.rbxlx document-window match, activation attempts, all six checks, runtime evidence, play-stop state, screenshot evidence, and defects."],
      domain:"technical",risk:"high",reversible:true,
