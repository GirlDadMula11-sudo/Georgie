...
export function verifiedDirectResponse(input,toolResults=[]){
  ...
  if(macJobs){
    ...
    return{text:lines.join("\n"),responseId:null,webSearches:0,model:"deterministic-mac-jobs",terminalState:active.length?"in_progress":"verified",completed:active.length===0,route:{domain:"mac",tier:"fast",reasoningEffort:"none",latencyClass:"instant"}};
  }
  ...
  if(prepared){
    ...
    return{text:`Approval plan prepared.\n- Plan: ${plan.id||"unknown"}\n- Approval: ${approval.id||plan.approvalId||"unknown"}\n- Status: ${plan.status||approval.status||"awaiting_approval"}`,responseId:null,webSearches:0,model:"deterministic-approval-receipts",terminalState:"approval_needed",completed:false,route:{domain:"technical",tier:"fast",reasoningEffort:"none",latencyClass:"instant"}};
  }
  ...
  if(approvedPlan){
    ...
    return{text:`Approved plan receipt recorded.\n- Plan: ${plan.id||dispatch.planId||"unknown"}\n- Approval: ${plan.approvalId||dispatch.approvalId||"unknown"}\n- Status: ${status}`,responseId:null,webSearches:0,model:"deterministic-approval-receipts",terminalState:/completed|verified|accepted/.test(String(status))?"verified":"in_progress",completed:/completed|verified|accepted/.test(String(status)),route:{domain:"technical",tier:"fast",reasoningEffort:"none",latencyClass:"instant"}};
  }
...
export async function completeTurnV2({userId,sessionId,input,history=[],onProgress,shouldFinalize=()=>true}){const startedAt=Date.now();let firstResponseMs=0;
