...
async function macReceiptFastPath({userId,input,history,startedAt,progress}){
  const actions=deterministicToolPlanWithHistory(input,history);
  const allowed=new Set(["mac.job_receipt","mac.devices"]);
  if(!actions.length||actions.some(action=>!allowed.has(action?.tool))||!actions.some(action=>action?.tool==="mac.job_receipt"))return null;
  console.log(`[Georgie] governed tool plan ${JSON.stringify({source:"deterministic_mac_receipt",actionCount:actions.length,tools:actions.map(action=>action.tool)})}`);
  progress({type:"status",stage:"tool_running",message:"Reading the exact Mac job receipt…",tools:actions.map(action=>action.tool)});
  const toolResults=await Promise.all(actions.map(action=>executeTool({name:action.tool,args:action.args||{},userId,policy:"read"})));
  const response=humanizeResponse(verifiedDirectResponse(input,toolResults));
  const latencyMs=Date.now()-startedAt;
  const evidence=toolResults.map(result=>({source:result?.tool||"mac.read",observedAt:new Date().toISOString(),status:result?.ok===false?"failed":"observed"}));
  const actionSuccess=toolResults.every(result=>result?.ok===true);
  const result={...response,remembered:0,memoryCount:0,actions:toolResults,evidence,evidenceFreshness:"observed_this_turn",confidence:"evidence_backed",engine:"unified-georgie-runtime-v1-mac-receipt-fast-path",runtime:{version:"unified-georgie-runtime.v2-control-plane",objective:{domain:"mac",kind:"inspection",requiresTools:true},continuedFrom:null},latencyMs,firstResponseMs:latencyMs,contextReadyMs:latencyMs};
  progress({type:"delta",delta:response.text,text:response.text});
  progress({type:"complete",stage:response.terminalState||"verified",latencyMs,firstResponseMs:latencyMs,contextReadyMs:latencyMs,actionCount:toolResults.length,evidenceCount:evidence.length,actionSuccess});
  return result;
}
