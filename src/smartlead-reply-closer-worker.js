...
function replyHtml(job) {
  ...
  else if (job.reply_class === "call_request") html = `<p>Hi ${firstName},</p>${delayed}<p>Absolutely. We’d be glad to connect and walk through it directly. Send over a day and time that works well for you and we’ll coordinate the conversation.</p><p>${esc(disclosure)}</p><p>Best,<br>Georgie<br>Sierra Capital Advisory</p>`;
