...
let cacheLoaded=false,providerUnavailableUntil=0;const verifiedDevices=new Map();let cachePersistTail=Promise.resolve();
...
async function persistCache(){const work=cachePersistTail.catch(()=>{}).then(async()=>{let target=null;try{await fs.mkdir(path.dirname(cachePath),{recursive:true,mode:0o700});target=`${cachePath}.${process.pid}.${Date.now()}.${crypto.randomUUID()}.tmp`;const items=[...verifiedDevices.values()].filter(item=>Date.now()-new Date(item.verifiedAt).getTime()<cacheTtlMs);await fs.writeFile(target,JSON.stringify({version:1,items}),{mode:0o600});await fs.rename(target,cachePath);}catch(error){if(target)await fs.unlink(target).catch(()=>{});console.warn("Native auth cache write failed:",error instanceof Error?error.message:error);}});cachePersistTail=work;return work;}
