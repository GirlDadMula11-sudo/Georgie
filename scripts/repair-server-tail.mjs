...
if (source.includes(marker) && !hasCompleteServerTail) {
  ...
  console.log('[Georgie] server.js tail already intact; preserving all registered routes and workers.');
}

const v2Import = 'import { completeTurnV2 } from "./v2-turn-engine.js";';
const realtimeImport = 'import { attachRealtimeRelay } from "./realtime-relay.js";';
const anchor = 'import { createSierraRouter } from "./sierra-router.js";';
if (!source.includes(v2Import) || !source.includes(realtimeImport)) {
  if (!source.includes(anchor)) throw new Error('Unable to locate Georgie server import anchor');
  const additions = [!source.includes(v2Import) ? v2Import : null, !source.includes(realtimeImport) ? realtimeImport : null].filter(Boolean).join('\n');
...
if (!source.includes('engine:"v2-concurrent"')) {
  ...
  const protectedRouteAnchors = [
    'app.use("/api/ai-control/receipt-relay"',
    'app.use("/api/ai-control/inbound"',
    'app.use("/api/connector"'
  ];
  const routeIndexes = protectedRouteAnchors.map(value => source.indexOf(value, start)).filter(index => index !== -1);
  const end = routeIndexes.length ? Math.min(...routeIndexes) : source.indexOf('app.get("/health"', start);
  ...
  changed = true;
  console.log('[Georgie] Activated v2 concurrent turn engine while preserving registered AI-control routes.');
}

if (!source.includes('attachRealtimeRelay(server)')) {
  const oldListen = 'app.listen(PORT,()=>console.log(`Georgie listening on port ${PORT}`));';
  if (source.includes(oldListen)) {
    source = source.replace(oldListen, 'const server=app.listen(PORT,()=>console.log(`Georgie listening on port ${PORT}`));\nattachRealtimeRelay(server);');
    changed = true;
  }
}
