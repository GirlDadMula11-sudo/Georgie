...
if (
  source.includes('READ_CACHE_TTL_MS') &&
  source.includes('identicalCleanWritesSuppressed:true') &&
  source.includes('queuedRequestCircuitFence:true') &&
  source.includes('sameState(existing.state,state)') &&
  source.includes('await acquire();try{if(Date.now()<providerUnavailableUntil)')
) {
  console.log('[Georgie] cloud-state pressure hardening already installed: changed=false');
  process.exit(0);
}
...
replaceRequired(
  ...
  'async function performWrite(userId,namespace,state){const key=stateKey(userId,namespace),existing=await readMirror(userId,namespace);if(existing&&sameState(existing.state,state)){if(existing.dirty){recentReads.delete(key);scheduleDirtyRecovery(userId,namespace,state);}else cacheState(key,state);return true;}const locallyDurable=await writeMirror(userId,namespace,state,true);if(locallyDurable&&!existing?.dirty)pendingWrites+=1;if(Date.now()<providerUnavailableUntil){recentReads.delete(key);return locallyDurable;}try{await putCloud(userId,namespace,state);cacheState(key,state);return true;}catch(error){recentReads.delete(key);lastError=errorMessage(error);console.warn(`Cloud state write deferred for ${namespace}:`,lastError);return locallyDurable;}}',
