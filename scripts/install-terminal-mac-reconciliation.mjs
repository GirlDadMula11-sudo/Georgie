...
for (const marker of [manifestImport, "authoritativeJob", "server_live_capability_manifest", "inspectionResultReturned", "liveManifestVerified", "response.reconciliationScheduled=true", "const requeue=()=>{if(!schedule(userId,command)", "certificationGate:item.certificationGate", "unified-georgie-runtime\\.v[a-z0-9]+(?:-[a-z0-9]+)*"]) {
...
console.log("[Georgie] authoritative terminal Mac reconciliation + certification-priority recovery + normalized manifest gate + singleflight-safe retry + restart-safe resume installed");
