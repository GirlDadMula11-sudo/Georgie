...
if (governed.includes('.slice(0,12),scheduled=[]')) {
  governed = governed.replace('.slice(0,12),scheduled=[]', '.slice(0,3),scheduled=[]');
...
if (!finalGoverned.includes('.slice(0,3),scheduled=[]')) throw new Error("CONNECTOR_RECOVERY_SUPERVISOR_CONCURRENCY_VERIFY_FAILED");
console.log("[Georgie] persistent connector recovery supervisor installed with bounded reconciliation concurrency=3");
